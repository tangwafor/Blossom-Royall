...
CREATE POLICY fulfillment_events_read ON public.order_fulfillment_events FOR SELECT TO authenticated USING (((private.current_store_role(store_id) IS NOT NULL) OR (EXISTS ( SELECT 1
   FROM public.orders order_record
  WHERE ((order_record.id = order_fulfillment_events.order_id) AND (order_record.customer_id = ( SELECT auth.uid() AS uid))))) OR (EXISTS ( SELECT 1
   FROM public.order_items item
  WHERE ((item.order_id = item.order_id) AND private.owns_vendor(item.vendor_id))))));
...
CREATE POLICY inventory_movements_read ON public.inventory_movements FOR SELECT TO authenticated USING ((private.current_store_role(store_id) IS NOT NULL));
...
CREATE POLICY products_access ON public.products FOR SELECT TO authenticated USING (((private.current_store_role(store_id) IS NOT NULL) OR ((status = 'published'::text) AND private.is_published_customer(store_id))));
...
CREATE POLICY variants_access ON public.product_variants FOR SELECT TO authenticated USING (((private.current_store_role(private.product_store(product_id)) IS NOT NULL) OR (EXISTS ( SELECT 1
   FROM public.products product
  WHERE ((product.id = product_variants.product_id) AND (product.status = 'published'::text) AND private.is_published_customer(product.store_id))))));
...
CREATE POLICY vendor_storefronts_read ON public.vendor_storefronts FOR SELECT TO authenticated USING (((private.current_store_role(store_id) IS NOT NULL) OR private.owns_vendor(vendor_id) OR ((status = 'published'::text) AND private.is_published_customer(store_id))));
...
CREATE POLICY vendors_read_store ON public.vendors FOR SELECT TO authenticated USING (((private.current_store_role(store_id) IS NOT NULL) OR ((status = 'active'::text) AND private.is_published_customer(store_id))));
