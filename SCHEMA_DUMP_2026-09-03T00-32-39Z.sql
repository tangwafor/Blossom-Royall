...
CREATE TABLE public.cash_drawer_sessions (
    ...
    CONSTRAINT cash_drawer_sessions_closing_valid CHECK ((((status = 'open'::text) AND (expected_cash IS NULL) AND (counted_cash IS NULL) AND (variance IS NULL) AND (closed_by IS NULL) AND (closed_at IS NULL)) OR ((status = 'closed'::text) AND (expected_cash IS NOT NULL) AND (counted_cash >= (0)::numeric) AND (variance = (counted_cash - expected_cash)) AND (closed_at IS NOT NULL)))),
    CONSTRAINT cash_drawer_sessions_note_length CHECK (((length(opening_note) <= 500) AND (length(closing_note) <= 500))),
    CONSTRAINT cash_drawer_sessions_opening_valid CHECK ((opening_float >= (0)::numeric)),
    CONSTRAINT cash_drawer_sessions_status_valid CHECK ((status = ANY (ARRAY['open'::text, 'closed'::text])))
);
...
CREATE FUNCTION public.close_cash_drawer(p_session_id uuid, p_counted_cash numeric, p_note text DEFAULT ''::text) RETURNS public.cash_drawer_sessions
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'private', 'pg_temp'
    AS $$
declare
...
--
-- Name: get_vendor_order_summaries(uuid); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.get_vendor_order_summaries(p_store_id uuid) RETURNS TABLE(id uuid, attributed_total numeric, status text, fulfillment_method text, fulfillment_status text, payment_status text, created_at timestamp with time zone)
    LANGUAGE sql STABLE SECURITY DEFINER
    SET search_path TO ''
    AS $$
  select
...
--
-- Name: open_cash_drawer(uuid, uuid, numeric, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.open_cash_drawer(p_store_id uuid, p_register_id uuid, p_opening_float numeric, p_note text DEFAULT ''::text) RETURNS public.cash_drawer_sessions
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'private', 'pg_temp'
    AS $$
declare
...
--
-- Name: cash_drawer_adjustments; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.cash_drawer_adjustments (
    ...
    CONSTRAINT cash_drawer_adjustments_amount_valid CHECK ((amount > (0)::numeric)),
    CONSTRAINT cash_drawer_adjustments_reason_present CHECK (((length(TRIM(BOTH FROM reason)) >= 3) AND (length(TRIM(BOTH FROM reason)) <= 240))),
    CONSTRAINT cash_drawer_adjustments_type_valid CHECK ((adjustment_type = ANY (ARRAY['paid_in'::text, 'paid_out'::text])))
);


--
-- Name: record_cash_drawer_adjustment(uuid, text, numeric, text); Type: FUNCTION; Schema: public; Owner: -
--

CREATE FUNCTION public.record_cash_drawer_adjustment(p_session_id uuid, p_adjustment_type text, p_amount numeric, p_reason text) RETURNS public.cash_drawer_adjustments
    LANGUAGE plpgsql SECURITY DEFINER
    SET search_path TO 'public', 'private', 'pg_temp'
    AS $$
declare
...
CREATE TABLE public.cash_registers (
    ...
    CONSTRAINT cash_registers_name_present CHECK (((length(TRIM(BOTH FROM name)) >= 1) AND (length(TRIM(BOTH FROM name)) <= 80)))
);
...
--
-- Name: store_operating_settings; Type: TABLE; Schema: public; Owner: -
--
...
CREATE TABLE public.store_operating_settings (
    ...
    owner_display_name text DEFAULT ''::text NOT NULL,
    operating_address text DEFAULT ''::text NOT NULL,
    receipt_phone text DEFAULT ''::text NOT NULL,
    receipt_email text DEFAULT ''::text NOT NULL,
    locale text DEFAULT 'en-US'::text NOT NULL,
    timezone text DEFAULT 'America/New_York'::text NOT NULL,
    order_prefix text DEFAULT 'BR'::text NOT NULL,
    retail_policy jsonb DEFAULT '{}'::jsonb NOT NULL,
    commerce_controls jsonb DEFAULT '{}'::jsonb NOT NULL,
    delivery_controls jsonb DEFAULT '{}'::jsonb NOT NULL,
    created_by uuid,
    updated_by uuid,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT store_operating_address_length CHECK ((length(operating_address) <= 500)),
    CONSTRAINT store_operating_commerce_object CHECK ((jsonb_typeof(commerce_controls) = 'object'::text)),
    CONSTRAINT store_operating_delivery_object CHECK ((jsonb_typeof(delivery_controls) = 'object'::text)),
    CONSTRAINT store_operating_email_valid CHECK (((receipt_email = ''::text) OR (receipt_email ~* '^[^@[:space:]]+@[^@[:space:]]+\.[^@[:space:]]+$'::text))),
    CONSTRAINT store_operating_legal_name_length CHECK ((length(legal_name) <= 180)),
    CONSTRAINT store_operating_locale_present CHECK (((length(TRIM(BOTH FROM locale)) >= 2) AND (length(TRIM(BOTH FROM locale)) <= 35))),
    CONSTRAINT store_operating_order_prefix_valid CHECK ((order_prefix ~ '^[A-Z0-9]{2,8}$'::text)),
    ...
    CONSTRAINT store_operating_policy_object CHECK ((jsonb_typeof(retail_policy) = 'object'::text)),
    CONSTRAINT store_operating_public_name_present CHECK (((length(TRIM(BOTH FROM public_name)) >= 1) AND (length(TRIM(BOTH FROM public_name)) <= 120))),
    CONSTRAINT store_operating_timezone_present CHECK (((length(TRIM(BOTH FROM timezone)) >= 3) AND (length(TRIM(BOTH FROM timezone)) <= 80)))
);
...
--
-- Name: cash_drawer_adjustments cash_drawer_adjustments_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_drawer_adjustments
    ADD CONSTRAINT cash_drawer_adjustments_pkey PRIMARY KEY (id);
...
--
-- Name: cash_registers cash_registers_store_id_name_key; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_registers
    ADD CONSTRAINT cash_registers_store_id_name_key UNIQUE (store_id, name);
...
--
-- Name: store_operating_settings store_operating_settings_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.store_operating_settings
    ADD CONSTRAINT store_operating_settings_pkey PRIMARY KEY (store_id);
...
--
-- Name: cash_drawer_adjustments_session_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX cash_drawer_adjustments_session_idx ON public.cash_drawer_adjustments USING btree (session_id, recorded_at);


--
-- Name: cash_drawer_one_open_cashier_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX cash_drawer_one_open_cashier_idx ON public.cash_drawer_sessions USING btree (store_id, opened_by) WHERE ((status = 'open'::text) AND (opened_by IS NOT NULL));


--
-- Name: cash_drawer_one_open_register_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX cash_drawer_one_open_register_idx ON public.cash_drawer_sessions USING btree (register_id) WHERE (status = 'open'::text);


--
-- Name: cash_drawer_sessions_store_opened_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX cash_drawer_sessions_store_opened_idx ON public.cash_drawer_sessions USING btree (store_id, opened_at DESC);
...
--
-- Name: payments_drawer_session_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX payments_drawer_session_idx ON public.payments USING btree (drawer_session_id, received_at) WHERE (method = 'cash'::text);
...
--
-- Name: cash_drawer_adjustments cash_drawer_adjustments_audit; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER cash_drawer_adjustments_audit AFTER INSERT OR DELETE OR UPDATE ON public.cash_drawer_adjustments FOR EACH ROW EXECUTE FUNCTION private.audit_cash_operations();


--
-- Name: cash_drawer_sessions cash_drawer_sessions_audit; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER cash_drawer_sessions_audit AFTER INSERT OR DELETE OR UPDATE ON public.cash_drawer_sessions FOR EACH ROW EXECUTE FUNCTION private.audit_cash_operations();


--
-- Name: cash_registers cash_registers_audit; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER cash_registers_audit AFTER INSERT OR DELETE OR UPDATE ON public.cash_registers FOR EACH ROW EXECUTE FUNCTION private.audit_cash_operations();
...
--
-- Name: payments payments_assign_cash_drawer; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER payments_assign_cash_drawer BEFORE INSERT ON public.payments FOR EACH ROW EXECUTE FUNCTION private.assign_cash_drawer_session();
...
--
-- Name: store_operating_settings store_operating_settings_audit; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER store_operating_settings_audit AFTER INSERT OR DELETE OR UPDATE ON public.store_operating_settings FOR EACH ROW EXECUTE FUNCTION private.audit_store_operating_settings();
...
--
-- Name: cash_drawer_adjustments cash_drawer_adjustments_recorded_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_drawer_adjustments
    ADD CONSTRAINT cash_drawer_adjustments_recorded_by_fkey FOREIGN KEY (recorded_by) REFERENCES public.profiles(id) ON DELETE SET NULL;


--
-- Name: cash_drawer_adjustments cash_drawer_adjustments_session_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_drawer_adjustments
    ADD CONSTRAINT cash_drawer_adjustments_session_id_fkey FOREIGN KEY (session_id) REFERENCES public.cash_drawer_sessions(id) ON DELETE RESTRICT;


--
-- Name: cash_drawer_adjustments cash_drawer_adjustments_store_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_drawer_adjustments
    ADD CONSTRAINT cash_drawer_adjustments_store_id_fkey FOREIGN KEY (store_id) REFERENCES public.stores(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.cash_drawer_sessions
    ADD CONSTRAINT cash_drawer_sessions_closed_by_fkey FOREIGN KEY (closed_by) REFERENCES public.profiles(id) ON DELETE SET NULL;


--
-- Name: cash_drawer_sessions cash_drawer_sessions_opened_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_drawer_sessions
    ADD CONSTRAINT cash_drawer_sessions_opened_by_fkey FOREIGN KEY (opened_by) REFERENCES public.profiles(id) ON DELETE SET NULL;


--
-- Name: cash_drawer_sessions cash_drawer_sessions_register_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_drawer_sessions
    ADD CONSTRAINT cash_drawer_sessions_register_id_fkey FOREIGN KEY (register_id) REFERENCES public.cash_registers(id) ON DELETE RESTRICT;


--
-- Name: cash_drawer_sessions cash_drawer_sessions_store_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_drawer_sessions
    ADD CONSTRAINT cash_drawer_sessions_store_id_fkey FOREIGN KEY (store_id) REFERENCES public.stores(id) ON DELETE CASCADE;


--
-- Name: cash_registers cash_registers_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_registers
    ADD CONSTRAINT cash_registers_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.profiles(id) ON DELETE SET NULL;


--
-- Name: cash_registers cash_registers_store_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_registers
    ADD CONSTRAINT cash_registers_store_id_fkey FOREIGN KEY (store_id) REFERENCES public.stores(id) ON DELETE CASCADE;


--
-- Name: cash_registers cash_registers_updated_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.cash_registers
    ADD CONSTRAINT cash_registers_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.profiles(id) ON DELETE SET NULL;
...
--
-- Name: payments payments_drawer_session_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.payments
    ADD CONSTRAINT payments_drawer_session_id_fkey FOREIGN KEY (drawer_session_id) REFERENCES public.cash_drawer_sessions(id) ON DELETE RESTRICT;
...
--
-- Name: store_operating_settings store_operating_settings_created_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.store_operating_settings
    ADD CONSTRAINT store_operating_settings_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.profiles(id) ON DELETE SET NULL;


--
-- Name: store_operating_settings store_operating_settings_store_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.store_operating_settings
    ADD CONSTRAINT store_operating_settings_store_id_fkey FOREIGN KEY (store_id) REFERENCES public.stores(id) ON DELETE CASCADE;


--
-- Name: store_operating_settings store_operating_settings_updated_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.store_operating_settings
    ADD CONSTRAINT store_operating_settings_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public.profiles(id) ON DELETE SET NULL;
...
--
-- Name: cash_drawer_adjustments; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.cash_drawer_adjustments ENABLE ROW LEVEL SECURITY;

--
-- Name: cash_drawer_adjustments cash_drawer_adjustments_read; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY cash_drawer_adjustments_read ON public.cash_drawer_adjustments FOR SELECT TO authenticated USING ((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role, 'staff'::public.app_role])));


--
-- Name: cash_drawer_adjustments cash_drawer_adjustments_remove; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY cash_drawer_adjustments_remove ON public.cash_drawer_adjustments FOR DELETE TO authenticated USING (((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role])) AND (EXISTS ( SELECT 1
   FROM public.cash_drawer_sessions session
  WHERE ((session.id = cash_drawer_adjustments.session_id) AND (session.status = 'open'::text))))));
...
--
-- Name: cash_drawer_sessions cash_drawer_sessions_read; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY cash_drawer_sessions_read ON public.cash_drawer_sessions FOR SELECT TO authenticated USING ((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role, 'staff'::public.app_role])));


--
-- Name: cash_drawer_sessions cash_drawer_sessions_remove_unused; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY cash_drawer_sessions_remove_unused ON public.cash_drawer_sessions FOR DELETE TO authenticated USING (((private.current_store_role(store_id) = 'owner'::public.app_role) AND (status = 'open'::text)));
...
--
-- Name: cash_registers cash_registers_create; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY cash_registers_create ON public.cash_registers FOR INSERT TO authenticated WITH CHECK (((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role])) AND (created_by = auth.uid()) AND (updated_by = auth.uid())));


--
-- Name: cash_registers cash_registers_read; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY cash_registers_read ON public.cash_registers FOR SELECT TO authenticated USING ((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role, 'staff'::public.app_role])));


--
-- Name: cash_registers cash_registers_remove; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY cash_registers_remove ON public.cash_registers FOR DELETE TO authenticated USING ((private.current_store_role(store_id) = 'owner'::public.app_role));


--
-- Name: cash_registers cash_registers_update; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY cash_registers_update ON public.cash_registers FOR UPDATE TO authenticated USING ((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role]))) WITH CHECK (((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role])) AND (updated_by = auth.uid())));
...
CREATE POLICY orders_access ON public.orders FOR SELECT TO authenticated USING (((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role, 'staff'::public.app_role])) OR (customer_id = ( SELECT auth.uid() AS uid))));
...
--
-- Name: store_operating_settings; Type: ROW SECURITY; Schema: public; Owner: -
--

ALTER TABLE public.store_operating_settings ENABLE ROW LEVEL SECURITY;

--
-- Name: store_operating_settings store_operating_settings_create; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY store_operating_settings_create ON public.store_operating_settings FOR INSERT TO authenticated WITH CHECK (((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role])) AND (created_by = auth.uid()) AND (updated_by = auth.uid())));


--
-- Name: store_operating_settings store_operating_settings_read; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY store_operating_settings_read ON public.store_operating_settings FOR SELECT TO authenticated USING ((private.current_store_role(store_id) IS NOT NULL));


--
-- Name: store_operating_settings store_operating_settings_remove; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY store_operating_settings_remove ON public.store_operating_settings FOR DELETE TO authenticated USING ((private.current_store_role(store_id) = 'owner'::public.app_role));


--
-- Name: store_operating_settings store_operating_settings_update; Type: POLICY; Schema: public; Owner: -
--

CREATE POLICY store_operating_settings_update ON public.store_operating_settings FOR UPDATE TO authenticated USING ((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role]))) WITH CHECK (((private.current_store_role(store_id) = ANY (ARRAY['owner'::public.app_role, 'manager'::public.app_role])) AND (updated_by = auth.uid())));
