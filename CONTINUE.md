...
The disposable production fixture foundation is implemented in `scripts/role-course-fixtures.mjs`. It creates isolated owner, manager, staff, vendor, and customer accounts plus a dedicated QA vendor, published ring with five stock units, and signed lease under a unique run identifier. Live production validation created every fixture, removed dependent inventory movements, lease records, catalog records, audit rows, users, and vendor, then passed the final leak audit. The recorder now resolves a separate sealed backend user identifier for every selected role. Protected checkout generated orders and returns are the next fixture layer.
