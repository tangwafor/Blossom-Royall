...
| Atomic checkout persistence | Fresh production schema verified August 28, 2026 with no drift. The staged transaction locks requested variants, rejects insufficient stock, calculates prices from database records, creates seller attributed order lines, records cash or pending electronic tender, writes inventory movements and vendor ledger credits, issues a sequential receipt, and records an audit event. Runtime proof upload cleans up orphaned evidence when checkout fails. Production activation remains pending migration approval, rollback validation, concurrency tests, and real payment authorization. |
